...
class S3Browser extends Component {
    ...
    async setFolder(folder) {
        ...
        // If the cache is undefined then try to get the information from the backend
            ...
            try {
                ...
                let response = await this.QASM.call_backend(window, function_names.GET_FOLDER_CONTENTS, folder);
    ...
    async createPath(final_segment, depth, cascade=false) {
        ...
        if (cascade) {
            ...
            const response = await this.QASM.call_backend(window, function_names.GET_FOLDER_CONTENTS, cascaded_path);
